...
def PredictandEvaluateModel (RF_model, test_set):
  ...
  return scoreAndLabelsPandas, precision, recall, f1, accuracy
...
def RFmodelPipeline(train_set, test_set, numTreeList, maxDepthList, maxBins = 400):
  ...
  for numTrees, maxDepth in zip(numTreeList, maxDepthList):
    ...
    scoreandLabels, precision, recall, f1, accuracy = PredictandEvaluateModel (RF_model, test_set)
...
summary_table, scoreandLabels = RFmodelPipeline(train_RF_US, test_RF, [30], [15])
